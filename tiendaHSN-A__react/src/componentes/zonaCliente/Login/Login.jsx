...
function Login() {
  ...
  const { setCliente, setAccessToken, setRefreshToken } = useGlobalState();
  ...
  async function handlerOnSubmit(ev) {
    try {
      ev.preventDefault();
      console.log(
        `Datos del formulario de login: ${JSON.stringify(formLogin)}`
      );

      let petNodeLogin = await fetch(
      ...
      console.log(
        `Respuesta del servidor al intentar loguearse: ${JSON.stringify(
          resNodeLogin
        )}`
      );
      if (resNodeLogin.codigo !== 0)
        throw new Error(
          `No se ha podido iniciar sesión, error: ${resNodeLogin.mensaje}`
        );
      ...
      setAccessToken(resNodeLogin.accessToken);
      setRefreshToken(resNodeLogin.refreshToken);
      ...
      <div className="border p-4">
        <div className="row">
          ...
          <div className="col-lg-6">
            ...
            <p className="mb-4">
              Si ya eres usuario registrado, introduce tu email y la contraseña
              que utilizaste en el registro
            </p>
            <form
            ...
            >
              {["email", "password"].map((el, pos) => (
                <HTMLInput
                  ...
                  labelInput={
                    el === "email" ? "Correo electrónico" : "Contraseña"
                  }
          ...
          <div className="col-lg-6 mt-4 mt-lg-0">
            <div className="p-4 bg-light h-100">
              ...
              <p className="mb-3">
                ...
                sorpresa... todas estas ventajas y muchas más con la cuenta HSN
              </p>

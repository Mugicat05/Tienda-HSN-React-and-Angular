...
se exporta un array de objetos "Route": https://angular.dev/api/router/Route
...
loadComponent: () => Promise<Type<any>> <---- funcion que carga dinamicamente los componentes segun se demanda: LAZY
...
redirectTo: string <---- ruta a la que se redirige cuando se navega a esta ruta
    ...
    COMANDO:   ng generate resolver listaProductos --flat --skip-tests


otras propiedades importantes:
    -resolve: ResolveData <--- objeto que define los RESOLVER (loadera de React) funciones que se ejecutan antes de activar la ruta para
                                ...
                                para recuperar los datos del componente necesitas el servicio ActivatedRoute


     - canActivate, canActivateChild, .... <---- son arrays de funciones GUARD; un "guard" es una funcion que bloquea el acceso
                                                 a una ruta en funcion del codigo que tiene implementado; si devuelve:
